##########################################################################
#1. Library size in terms of umis
#2. by counting unique genes detected in each sample
filter_instances_variables<- function(total_values,output_pdf_name,output_pdf_name_2) {
  lista_dataframes = list()
  #
  pdf(file=output_pdf_name)
  for (n in 1:length(file_list)){
    min<-0
    max<-max(list_all[[n]][[total_values]])
    gup<-quantile(list_all[[n]][[total_values]],probs=(.9))
    gdown<-quantile(list_all[[n]][[total_values]],probs=(.1))
    hist(
      main=file_list[n],
      list_all[[n]][[total_values]],
      breaks = 100,
      xlim=c(min,max),
      xlab="Total counts"
    )
    abline(v = c(gdown,gup), col = "red")
    #Filter library size
    filter_by <- (list_all[[n]][[total_values]] > gdown & list_all[[n]][[total_values]] < gup)
    assign(paste(file_list[n],"filter_by",sep = "_"), filter_by)
    
    #Intialize
    lista_dataframes = append(lista_dataframes, list(get(paste(file_list[n],"filter_by",sep = "_"))))
    
    print (sprintf("Filtering %s plots_Histograms added to pdf -> %s", file_list[n],output_pdf_name))
  }
  dev.off()
  
  #Export tables with results
  pdf(file=output_pdf_name_2)
  for (n in 1:length(file_list)){
    #Plot table with amount of elements filtered
    df=knitr::kable(
      as.data.frame(table(get(paste(file_list[n],"filter_by",sep = "_")))),
      booktabs = TRUE,
      row.names = FALSE,
      caption = 'Filtered out (FALSE)',
      col.names = c(paste(file_list[n],"filter_by",sep = "_"), "Freq")
    )
    #Export tables
    p<-tableGrob(df)
    grid.arrange(p)
    
    print (sprintf("Saving %s table of filtered out elements to pdf -> %s", file_list[n],output_pdf_name))
  }
  dev.off()
  
  return(lista_dataframes)
}

##########################################################################
#Fuction for filter ERCC and MT
filter_ERCC_MT<- function(is_spike) {
  lista_filtrada = list()
  #
  for (n in 1:length(file_list)){
    #ERCC
    filtrada_by <- list_all[[n]]@int_elementMetadata@listData[[is_spike]]
    assign(paste(file_list[n],"filtrada_by",sep = "_"), filtrada_by)
    lista_filtrada = append(lista_filtrada, list(get(paste(file_list[n],"filtrada_by",sep = "_"))))
  }
  
  return(lista_filtrada)
}

##########################################################################
#Change names
name_changer<-function(especie_database, especie_name, x){
  ensembl <- useMart(especie_database, dataset = especie_name)

  annotation = getBM(attributes=c("external_gene_name","entrezgene"), values=x, mart=ensembl)
  
  return(annotation)
}

##########################################################################
#GO UNIVERSE
create_GO_universe<-function(especie_database, especie_name){
  frame = toTable(especie_database)
  goframeData = data.frame(frame$go_id, frame$Evidence, frame$gene_id)
  head(goframeData)
  
  goFrame=GOFrame(goframeData,organism = especie_name)
  goAllFrame=GOAllFrame(goFrame)
  gsc.GO = GeneSetCollection(goAllFrame, setType = GOCollection()) #takes around 6-8 min

  return(gsc.GO)
}

##########################################################################
#KEGG UNIVERSE
create_KEGG_universe<-function(especie_database, especie_name){
  frame = toTable(especie_database)
  keggframeData = data.frame(frame$path_id, frame$gene_id)
  
  keggFrame=KEGGFrame(keggframeData,organism = especie_name)
  gsc.KEGG = GeneSetCollection(keggFrame, setType = KEGGCollection())
  
  return(gsc.KEGG)
}

##########################################################################
#GSEA HyperG
gsea_HyperG_GO<-function(name_tool, gene_colection , annotations, ontology, cutoff){
  params = GSEAGOHyperGParams(name=name_tool,  
                                    geneSetCollection=gene_colection, 
                                    geneIds = annotations, 
                                    universeGeneIds = universe,  
                                    ontology = ontology, 
                                    pvalueCutoff = cutoff,  
                                    conditional = FALSE, 
                                    testDirection = "over")
  results=hyperGTest(params)
  tmp=(summary(results))
  tmp$Pvalue=p.adjust(tmp$Pvalue,"BH")
  tmp=(subset(tmp,tmp$Pvalue<0.01))
  
  return(tmp)
}
gsea_HyperG_KEGG<-function(name_tool, gene_colection , annotations, cutoff){
  params <- GSEAKEGGHyperGParams(name=name_tool,  
                                      geneSetCollection=gene_colection, 
                                      geneIds = annotations, 
                                      universeGeneIds = universe,  
                                      pvalueCutoff = cutoff, 
                                      testDirection = "over")
  results=hyperGTest(params)
  tmp=(summary(results))
  tmp$Pvalue=p.adjust(tmp$Pvalue,"BH")
  tmp=(subset(tmp,tmp$Pvalue<0.01))
  
  return(tmp)  
}

##########################################################################
#Filter for selecting rows and column from count matrix
create_selection<-function(cluster_for_genes, cluster_for_cells){
  genes_to_be_used = subset(multiCCA.markers,(multiCCA.markers['cluster'] == c(cluster_for_genes)))
  genes_to_be_used = as.character(unlist(genes_to_be_used['gene']))
  genes_to_be_used = data_matrix[rownames(data_matrix) %in% genes_to_be_used,]
  
  cells_to_be_used = subset(cluster_ident,(cluster_ident['multiCCA.ident'] == c(cluster_for_cells)))
  cells_to_be_used = as.character(unlist(cells_to_be_used['Cells']))
  cells_to_be_used = gsub("-", ".", cells_to_be_used)
  
  aracne_use_cluster = genes_to_be_used[colnames(genes_to_be_used) %in% cells_to_be_used]
  
  return(aracne_use_cluster)
}